...
public class edit_products_menu extends AppCompatActivity {
    private static final int REC_RESULT = 653;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_edit_products_menu);
    }
    ...
    public void delete_product(View view){
        Intent intent=new Intent(getApplicationContext(),delete_products.class);
        startActivity(intent);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {//SEARCHING THE USER'S VOICE COMMANDS
        ...
        if (requestCode==REC_RESULT && resultCode==RESULT_OK) {
            ...
            else if(matches.contains("δύο")||matches.contains("2")||matches.contains("two")){Intent intent=new Intent(getApplicationContext(),add_products.class);


                startActivity(intent);}

            else if(matches.contains("τρία")||matches.contains("3")||matches.contains("three")){Intent intent=new Intent(getApplicationContext(),delete_products.class);
